<template>
  <en-dialog @close="closeModal" :loading="loading" width="660px" destroy-on-close>
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <el-form-item prop="xb" label="性别">
            <el-select v-model="ruleForm.xb" placeholder="请选择" size="large">
              <el-option v-for="item in xbList" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item prop="zymc" label="专业">
            <el-input size="large" v-model="ruleForm.zymc" placeholder="请输入" />
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item prop="nj" label="年级">
            <el-select v-model="ruleForm.nj" placeholder="请选择" size="large">
              <el-option v-for="item in njList" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item prop="xq" label="所在校区">
            <el-select v-model="ruleForm.xq" placeholder="请选择" size="large">
              <el-option v-for="item in xqList" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item prop="userType" label="学生类别">
            <el-select v-model="ruleForm.userType" placeholder="请选择" size="large">
              <el-option v-for="item in userTypeList" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item prop="xwlb" label="学位类别">
            <el-select v-model="ruleForm.xwlb" placeholder="请选择" size="large">
              <el-option v-for="item in xwlbList" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item prop="xxxs" label="学习形式">
            <el-select v-model="ruleForm.xxxs" placeholder="请选择" size="large">
              <el-option v-for="item in xxxsList" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="24">
...
const generateNjColumns = () => {
  ...
  for (let i = 0; i <= 6; i++) {
    ...
    columns.push({ label: `${year}级`, value: `${year}` });
  }
  return columns
}
const njList = generateNjColumns()
const cascaderProps = {
  multiple: true,
}
...
const { departmentTreeList,
  ...
  userTypeList,
  xqList
}: any = storeToRefs(useBaseFieldStore());
const roleList = computed(() => {
  return role?.value === 'system' ? roleDepartmentList.value : role?.value === 'student' ? roleStudentList.value : role?.value === 'student' ? roleTeacherList.value : [];
})
...
const rules: any = {
  ...
  zymc: [{ required: false, message: '请输入', trigger: 'blur' }],
  xq: [{ required: false, message: '请选择', trigger: 'change' }],
  userType: [{ required: false, message: '请选择', trigger: 'change' }],
  xxxs: [{ required: false, message: '请选择', trigger: 'change' }],
  xwlb: [{ required: false, message: '请选择', trigger: 'change' }],
  nj: [{ required: false, message: '请选择', trigger: 'change' }],
  roles: [{ required: true, message: '请选择', type: 'array', trigger: 'change' }],
  xb: [{ required: true, message: '请选择', trigger: 'change' }],
...
watch(() => props.visible,
  (value) => {
    if (value) {
      fetchBaseFieldEvent('xxxsList')
      fetchBaseFieldEvent('xwlbList')
      fetchBaseFieldEvent('userTypeList')
      fetchBaseFieldEvent('xqList')
    }
  }
)
</script>
