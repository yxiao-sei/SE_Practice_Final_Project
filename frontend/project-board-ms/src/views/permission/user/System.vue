<template>
  ...
  <update-item-modal role="system" :fillData="fillData" v-model="updateModal.visible" v-bind="updateModal"
...
const getDefaultFilters = async () => {
  const { roleDepartmentList }: any = await dispatchRoleList()
  return {
    filters: {
      "filters": {
        "$or": roleDepartmentList.map((item: any) => {
          return {
            "roles": {
              "$contains": item.value
            }
